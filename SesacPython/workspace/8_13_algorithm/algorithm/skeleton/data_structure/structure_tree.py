...
class Tree:
    ...
    def iter_nodes(self): # 노드만 iterator로 반환 # 클래스인스턴스의 재귀에 대한 반쪽짜리 이해
    ...
    def delete(self, address): # child 순환돌려서 idx 일치하면 del tree_lst[idx] 이후  .datum 반환 # t1.delete([1,2])
        res = []
        for idx,j in self.iter_nodes_with_address(): # 이러면 자식트리의 자식트리수정이 [1,2]
            if address == idx:
