...
def check_solution():
    from main import find_matching_pair, parse_parentheses

    for text in testcases():
        find_matching_pair(text, 0)
        parse_parentheses(text)

def check_skeleton():
    func_list = ['find_matching_pair', 'determine_if_rule0',
                 'determine_if_rule1', 'determine_if_rule2',
                 'parse_empty_string', 'default_node_information',
                 'update_rule1_data', 'update_rule1_mid',
                 'update_rule2_data', 'update_rule2_nodes']
    
    for idx, func in enumerate(func_list):
        exec(f'from main import {func} as a{str(idx)}')
